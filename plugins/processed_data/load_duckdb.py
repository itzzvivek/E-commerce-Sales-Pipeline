...
def load_parquet_to_duckdb(client, bucket_name, input_object, output_db="mnt/data/ecommerce.duckdb", table_name=None, **kwargs):
    if table_name is None:
        table_name = input_object.split("/")[-1].split('.')[0].lower().replace(" ", "_").replace("-", "_")
    ...
    return {"rows": len(df), "table": table_name, "db": output_db}